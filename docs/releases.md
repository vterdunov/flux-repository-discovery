...
| GitHub Release | Tag, notes and downloads | `0.1.0` |
...
- Git tags use `MAJOR.MINOR.PATCH`, for example `0.1.0`, without a `v` prefix.
  Build metadata (`+...`) is not supported because container tags cannot contain `+`.
- A release image uses the same version as its Git tag, for example `:0.1.0`.
...
- Prereleases such as `0.2.0-rc.1` publish `:0.2.0-rc.1` and are marked as
...
5. Verify the release assets and container version:
   ...
   ```sh
   gh release view 0.1.0 --repo vterdunov/flux-repository-discovery
